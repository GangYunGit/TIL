...
- TDZ란
  ...
  - Temporal Dead Zone
  - 변수가 선언되고 초기화 단계에 접어들기 전까지의 구간으로, 이 구간에 있는 변수를 참조하려고 하면 Reference Error가 발생함
  - javascript에서는 변수를 선언 -> 초기화 -> 할당하는 단계를 거친다. 특히 let, const키워드로 선언된 변수의 경우 실행 컨텍스트 단계에서 렉시컬 환경에 등록되는 과정까지만 진행되며, 메모리를 할당받는 초기화 과정은 이후 런타임에서 진행되기 때문에 변수의 주소값이 존재하지 않으므로 이 때 참조를 시도하게 되면 Reference에러가 발생하는 것.

- TDZ 예시

  ```javascript
  // 변수를 선언하기 전에 호출한 경우(let, const)
  console.log(num); // Uncaught ReferenceError ReferenceError: num is not defined
  const a = 10;
  ```

  ```javascript
  // Class를 선언하기 전에 호출한 경우
  const me = new Car('Lamborghini'); // Uncaught ReferenceError ReferenceError: Cannot access 'Car' before initialization
  class Car {
    constructor(name) {
      this.name = name;
    }
...
- TDZ는 코드가 작성된 순서가 아니라 초기화 시점과 관련이 있다.
...
- ES8에서 도입된 문법으로, 기존의 콜백함수나 Promise객체에서 비동기 처리를 수행할 때의 단점을 보완하기 위해 도입됨

- 비동기 요청을 순차적으로 처리하려면

  - 콜백함수의 경우에는 비동기 요청의 처리 결과를 내부에서 또 다시 콜백함수로 받아 사용하게되는 "콜백 헬"이 발생
  - Promise 객체를 이용하는 경우 then메서드를 이용하여 순차적으로 처리할 수 있으나, then으로 처리하는 작업이 많아지면 콜백 헬처럼 가독성이 나빠짐

- async / await를 이용하면 기존에 프로그래밍하던 방식처럼 비동기 처리 결과를 동기적으로 처리할 수 있게(비동기 처리 결과를 변수에 할당하고, 이 변수의 결과에 따라 if문으로 조건을 분기 하는 등) 해줌.

- 또한 콜백 함수나 Promise에서 사용하지 못했던 try - catch문을 이용하여 에러를 처리할 수 있음. 특히, 동기적 코드와 비동기 처리 코드가 동시에 존재하는 경우 try - catch로 한 번에 잡아낼 수 있다는 것도 장점.
